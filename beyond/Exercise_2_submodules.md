...
> [!IMPORTANT]
> **Where you work:** `git_beyond/conference_submodule`, a copy of the sandbox you make in the
> first task below. This has nothing to do with the *git-course* repository you cloned: the
> sandbox lives in `git_beyond/`, a **separate** directory next to *git-course*. See
...
The sandbox is the repository `git_beyond/conference_planning`. We work on a **copy** of it, so
that the original stays usable for the later exercises. Three steps get you set up.

First, move to `git_beyond/`, the folder that *contains* the sandbox, so that you are not inside
any repository:
...
at `~/<path>/git_beyond/glossary-tool.git` - use that path as the URL. A local filesystem path
...
```plaintext
git submodule add ~/<path>/git_beyond/glossary-tool.git c2sm-info
```
...
git -C ~/<path>/git_beyond/glossary-tool.git log --oneline -3
```
